// <copyright file="EslDispatchFailedIntegrationHandler.cs" company="TeckLab">
...
/// <summary>
/// Consumes <see cref="EslDispatchFailedIntegrationEvent"/> emitted by a vendor ESL worker after
/// an unrecoverable dispatch failure, and transitions the matching <see cref="DisplayAssignment"/>
/// to <c>Failed</c>.
/// </summary>
public sealed partial class EslDispatchFailedIntegrationHandler
{
    ...
    private readonly ILogger<EslDispatchFailedIntegrationHandler> logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="EslDispatchFailedIntegrationHandler"/> class.
    /// </summary>
    ...
    public EslDispatchFailedIntegrationHandler(
        ...
        ILogger<EslDispatchFailedIntegrationHandler> logger)
    {
    ...
    /// <summary>
    /// Handles the dispatch-failed integration event.
    /// </summary>
    ...
    public async Task Handle(EslDispatchFailedIntegrationEvent integrationEvent, CancellationToken cancellationToken)
    {
        ...
        ErrorOr<Success> transition = assignment.MarkFailed(integrationEvent.Reason);
        ...
        DisplayOperationFailed operationFailed = new(
            integrationEvent.DisplayId,
            AssignOperationType,
            integrationEvent.FailedAt,
            integrationEvent.Reason);

        await this.messageBus.PublishAsync(operationFailed).ConfigureAwait(false);

        LogAssignmentFailed(this.logger, assignment.Id, integrationEvent.EslProvider, integrationEvent.Reason);
    }
    ...
    [LoggerMessage(EventId = 1, Level = LogLevel.Warning, Message = "EslDispatchFailed received for unknown assignment. AssignmentId={AssignmentId} DisplayId={DisplayId}")]
    private static partial void LogAssignmentNotFound(ILogger logger, Guid assignmentId, Guid displayId);

    [LoggerMessage(EventId = 2, Level = LogLevel.Warning, Message = "MarkFailed rejected for assignment {AssignmentId}: {Reason}")]
    private static partial void LogTransitionRejected(ILogger logger, Guid assignmentId, string reason);

    [LoggerMessage(EventId = 3, Level = LogLevel.Warning, Message = "Assignment {AssignmentId} marked Failed via provider {Provider}: {Reason}")]
    private static partial void LogAssignmentFailed(ILogger logger, Guid assignmentId, string provider, string reason);

    [LoggerMessage(EventId = 4, Level = LogLevel.Warning, Message = "Access point {AccessPointSerial} was not found while failing dispatch")]
