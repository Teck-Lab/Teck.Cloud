...
public sealed class ActivateLicenseCommandHandlerTests
{
    private readonly ILicenseWriteRepository _licenseRepository;
    private readonly ActivateLicenseCommandHandler _sut;

    public ActivateLicenseCommandHandlerTests()
    {
        _licenseRepository = Substitute.For<ILicenseWriteRepository>();
        _sut = new ActivateLicenseCommandHandler(_licenseRepository);
    }
    ...
    public async Task Handle_ShouldReturnSuccess_WhenValidCommandProvided()
    {
        ...
        var command = new ActivateLicenseCommand(license.Id);
    ...
    [Fact]
    public async Task Handle_ShouldReturnSuccess_WhenLicenseAlreadyActive()
    {
        var license = CreateLicense();
        license.Activate(); // ensure already active
